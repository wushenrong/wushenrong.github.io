# SPDX-FileCopyrightText: 2019 Sam Rayner
# SPDX-FileCopyrightText: 2019 Otto Urpelainen
#
...
module Jekyll
  def self.get_post_path(page_id, collections)
    ...
    collections.each do |collection|
      current_doc = collection.docs.find { |doc| doc.id == page_id }
      return "#{collection.label}/#{current_doc.data["slug"]}" unless doc.nil?
    end
  ...
  class AssetPathTag < Liquid::Tag
    ...
    def render(context)
      ...
      # strip filename
      path = File.dirname(path) if %r!\.\w+$!.match?(path)
